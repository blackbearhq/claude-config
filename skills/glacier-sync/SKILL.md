...
description: >
  ...
  Only activates in projects with a `.glacier.json` config file in the repo root.
  If the skill is not enabled or `.glacier.json` is absent, skip silently — never fail the workflow.
---
# Glacier Sync
...
**This skill is optional.** It enhances the workflow but is never required. If the skill is not loaded, not enabled, or `.glacier.json` is missing, the calling workflow (e.g. `/implement`) must continue without interruption.
...
2. A `.glacier.json` file exists in the project root:
...
If either condition is not met, skip silently — do not prompt the user to create the file or enable the skill. Do not error. Do not block the workflow.
