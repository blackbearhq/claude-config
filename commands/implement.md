...
After step 1, attempt step 1b: if glacier-sync is enabled and `GLACIER_ENABLED=true` + `GLACIER_PROJECT_ID` are set in the environment, move the linked card to In Progress. If glacier-sync is not available or env vars are missing, skip silently and continue to step 2.
